...
export function scoreFrame(params: {
  ...
  // 3) PD × (렌즈폭 + 브릿지) (15점)
  // 이 값이 PD와 가까울수록 눈동자가 렌즈의 광학 중심에 온다.
  const frameCenter = frame.lensWidth + frame.bridge;
  if (prescription?.pd) {
    const gap = Math.abs(frameCenter - prescription.pd);
    if (gap <= 2) {
      score += 15;
      pros.push(`렌즈폭+브릿지 ${frameCenter}mm가 PD ${prescription.pd}mm와 거의 일치해 광학 중심이 잘 맞습니다.`);
    } else if (gap <= 5) {
      score += 10;
    } else {
      score += 3;
      cons.push(
        `렌즈폭+브릿지 ${frameCenter}mm와 PD ${prescription.pd}mm 차이가 ${gap.toFixed(0)}mm라 눈동자가 렌즈 중심에서 벗어납니다.`
      );
    }
  } else {
