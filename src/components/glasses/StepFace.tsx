...
import { FACE_SHAPES, FACE_SHAPE_IDS, FRAME_SHAPE_LABEL } from "@/lib/glasses/faceShapes";
import { fileToScaledDataUrl } from "@/lib/glasses/image";
import type { FaceAnalysis, FaceShapeId } from "@/lib/glasses/types";
...
export default function StepFace({
  ...
  async function handleFile(file: File) {
    ...
    try {
      ...
      setPreview(dataUrl);
      const res = await fetch("/api/glasses/analyze-face", {
        method: "POST",
  ...
  return (
    <div className="space-y-5">
      ...
      <label className="flex items-start gap-2 text-sm text-havruta-800">
        <input
          type="checkbox"
          checked={consent}
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="rounded-2xl border border-havruta-200 bg-white p-4">
          <p className="mb-2 font-semibold text-havruta-900">1. 사진으로 분석하기</p>
          ...
          />
          {loading && <p className="mt-3 text-sm text-havruta-600">얼굴 비율을 재는 중…</p>}
          {preview && !loading && (
        ...
        <div className="rounded-2xl border border-havruta-200 bg-white p-4">
          <p className="mb-2 font-semibold text-havruta-900">2. 또는 얼굴형 직접 고르기</p>
          <p className="mb-3 text-sm text-havruta-700">
              ...
              <button
                ...
                className={`rounded-full border px-3 py-1.5 text-sm ${
                  face?.faceShape === id && face.source === "manual"
                    ...
                    : "border-havruta-200 text-havruta-800 hover:bg-havruta-50"
                }`}
              >
      ...
      {face && shapeInfo && (
        <div className="rounded-2xl border border-havruta-300 bg-havruta-50 p-5">
          <div className="flex flex-wrap items-baseline gap-2">
            <h3 className="text-xl font-bold text-havruta-900">{shapeInfo.label}</h3>
            <span className="text-sm text-havruta-600">
              {face.source === "ai"
                ? `AI 분석 · 신뢰도 ${Math.round(face.confidence * 100)}%`
                : "직접 선택"}
            </span>
          </div>
          <p className="mt-2 text-sm leading-relaxed text-havruta-800">{face.summary}</p>
          ...
          <dl className="mt-4 grid grid-cols-2 gap-x-4 gap-y-2 text-sm sm:grid-cols-4">
            <div>
              <dt className="text-havruta-600">얼굴 가로/세로</dt>
              <dd className="font-medium text-havruta-900">{face.widthToHeight.toFixed(2)}</dd>
            </div>
            <div>
              <dt className="text-havruta-600">얼굴 폭</dt>
              <dd className="font-medium text-havruta-900">{WIDTH_LABEL[face.faceWidth]}</dd>
            </div>
            <div>
              <dt className="text-havruta-600">콧대</dt>
              <dd className="font-medium text-havruta-900">{NOSE_LABEL[face.noseBridge]}</dd>
            </div>
            <div>
              <dt className="text-havruta-600">눈썹</dt>
              <dd className="font-medium text-havruta-900">{BROW_LABEL[face.browLine]}</dd>
            </div>
          </dl>

          <p className="mt-4 text-sm text-havruta-800">
            <strong>추천 원리</strong> — {shapeInfo.principle}
          </p>
          <p className="mt-2 text-sm text-havruta-800">
            잘 맞는 테:{" "}
          ...
          {face.noseBridge === "low" && (
            <p className="mt-3 rounded-lg bg-white px-3 py-2 text-sm text-havruta-800">
