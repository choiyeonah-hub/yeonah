...
## 🐝 왕벌의 비행 — 다섯 개의 꿀 왕국 (`/bee`)

실제 꿀벌의 일령별 직무를 그대로 따라가는 브라우저 게임입니다. 서버·DB 없이
캔버스에서만 돌아가고, 단일 HTML 한 장으로도 배포됩니다.

### 생애 구조

실제 꿀벌은 나이를 먹으면서 맡는 일이 바뀝니다(temporal polyethism).

1. **청소벌** (1~2일령) — 갓 나온 방을 닦는다
2. **육아벌** (3~11일령) — 애벌레를 먹이고, 왕대에는 로열젤리만 준다
3. **건축벌** (12~17일령) — 밀랍을 내어 저장권에 육각방을 짓는다
4. **경비벌** (18~21일령) — 동료를 모아 천적을 열구로 밀어낸다
5. **채집벌** (22일령~) — 꽃꿀을 저장권까지 나른다
6. **처녀왕** — 혼인비행
...
지역마다 하늘빛, 꽃 모양, 천적, 기후, 벌통 재료가 다릅니다. 위협은 모두 그 땅에서
실제로 벌을 괴롭히는 것들입니다.

| 왕국 | 나라 | 꽃 | 천적 | 기후 |
|---|---|---|---|---|
| 마누카 계곡 | 뉴질랜드 | 마누카 | 말벌 | 짧은 개화 |
| 아몬드 농장 | 미국 캘리포니아 | 아몬드 꽃 | 살포 드론 | 단일 재배 (농약 꽃 2배) |
| 시드르 골짜기 | 예멘 | 시드르 | 벌잡이새 | 혹서 (날개 힘 급감) |
| 타임 절벽 | 그리스 | 야생 타임 | 말벌 | 해풍 (비행이 밀림) |
| 아카시아 숲 | 대한민국 | 아까시나무 꽃 | 등검은말벌 | 외래 천적 (빠르고 많음) |

한 왕국을 마치면 **분봉**해서 다음 왕국이 열립니다(진행은 브라우저에 저장).

### 그림

전부 도트로 찍는다. 캔버스의 곡선은 안티에일리어싱을 피할 수 없어서, 스프라이트는
`src/lib/bee/sprites.ts` 에 문자 격자로 두고 사각형으로만 찍는다. 화소 하나가 화면
픽셀의 정수 배가 되도록 배율을 정수로 맞추고, 카메라 좌표도 정수로 고정한다 —
소수 자리에 있으면 움직일 때 화면 전체가 미세하게 떨린다.

### 그 밖에

- 스테이지·왕국마다 실제 벌 생태 **사실 카드**
- **농약 묻은 꽃**을 빨면 방향 감각을 잃습니다 (조작 반전)
- 엔딩에서 「왕벌의 비행」(림스키코르사코프, 퍼블릭 도메인)이 WebAudio로 연주
- **공유용 결과 카드**를 PNG로 생성 (이미지 복사 / 길게 눌러 저장)
- 조작: 방향키 비행 · Space 부스터 · 접촉하면 자동 처리 · R 다시 시작

### 구조

- `src/lib/bee/regions.ts` — 다섯 왕국의 데이터
- `src/lib/bee/world.ts` — 벌통 생성 (지역별 천적·농약 반영)
- `src/lib/bee/game.ts` — 비행 물리, 스테이지, 기후 효과, 렌더, 결과 카드
- `src/lib/bee/music.ts` — 효과음과 「왕벌의 비행」 합성
- `src/lib/bee/standalone.ts` + `styles.ts` — 화면 조립 (Next 페이지와 단일 HTML 공용)
- `npm run build:standalone` → `dist/manuka.html` (설치·서버 없이 도는 한 장)
