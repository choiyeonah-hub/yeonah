...
- **정자의 여행 게임(`/game`)**: 가족이 함께 웃으며 하는 생명 탄생 미니게임입니다. 화면을 위아래로
  쓸어(마우스·방향키도 가능) 정자를 조종해, 산성 방울·자궁경부의 점액 벽·백혈구·나팔관의 역류를
  피하며 난자까지 헤엄쳐 갑니다. 네 구간을 지날 때마다 생명 상식 한 줄이 뜨고, 도착하면
  "내가 태어나던 날" 이야기를 나눠 보라는 가족 질문으로 이어집니다. 서버·AI 없이 캔버스만으로
  동작하고, 최고 기록은 브라우저(localStorage)에 남습니다.
...
src/lib/spermGame.ts       # 정자의 여행 게임 규칙·구간·장애물 생성
src/app/game/page.tsx      # 정자의 여행 게임 화면(캔버스 렌더링 + 조작)
```
